...
| `agentDID`                 | `string`                       | unset                       | sign requests as the registered agent DID                         |
| `agentPrivateKey`          | `string`                       | unset                       | base64-encoded Ed25519 private key for the registered agent DID   |
...
| `OPENBOX_AGENT_DID`                    | registered agent DID used for request signing           | empty                   |
| `OPENBOX_AGENT_PRIVATE_KEY`            | base64-encoded Ed25519 private key for request signing  | empty                   |
...
`OPENBOX_AGENT_DID` and `OPENBOX_AGENT_PRIVATE_KEY` must be configured together. If either value is set without the other, config parsing fails. The private key is returned once when the agent is registered in OpenBox and should be stored in the agent runtime secret store.
...
| `agentDID` and `agentPrivateKey` | set for production agents                        | enables cryptographic DID request signing when OpenBox enforces agent identity |
